    ...
    <main className="min-h-screen bg-[#f5f6f8] text-[#101827]">
      ...
      <section className="max-w-[1280px] mx-auto p-8">
        <div className="bg-[#101b2d] text-white rounded-2xl p-5 mb-8 flex justify-between items-center shadow">
          ...
          <Link
            href="/garage"
            className="border border-slate-600 px-5 py-2 rounded-lg font-bold"
          >
            Change Vehicle
          </Link>
        </div>

        <h2 className="text-4xl font-extrabold mb-6">Shopping Cart</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            {cartItems.map((item) => (
              <div
                key={item.name}
                className="bg-white rounded-2xl shadow p-6 flex gap-6 items-center"
              >
                <img
                  src={item.image}
                  alt={item.name}
                ...
                <div className="flex-1">
                  ...
                  <div className="flex justify-between items-center mt-5">
                    <div className="bg-gray-100 rounded-lg flex items-center">
                      <button className="px-4 py-2 font-bold">−</button>
                      <span className="px-4 font-bold">{item.qty}</span>
                      <button className="px-4 py-2 font-bold">+</button>
                    </div>

                    <button className="text-gray-700 hover:text-red-600 font-semibold">
                      Remove
                    </button>
                  </div>
                </div>
          ...
          <aside className="lg:col-span-4">
            <div className="bg-white rounded-2xl shadow p-6 sticky top-24">
              ...
              <div className="space-y-4 text-gray-700">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <b className="text-black">$498.90</b>
                </div>
                ...
                <div className="flex justify-between">
                  <span>Taxes</span>
                  <b className="text-black">$42.41</b>
                </div>

                <div className="border-t pt-4 flex justify-between text-xl">
                  <span className="font-extrabold">Total</span>
                  <span className="font-extrabold">$541.31</span>
                </div>
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="flex gap-2 mt-2">
                  ...
                  />
                  <button className="bg-[#101b2d] text-white px-4 rounded-lg font-bold">
                    Apply
                  </button>
                </div>
              </div>
              ...
              <Link
                href="/checkout"
                className="w-full mt-6 bg-[#e8a88a] text-white py-4 rounded-xl font-extrabold text-lg block text-center"
              >
                Proceed to Checkout →
              </Link>
            ...
            {suggestions.map((item) => (
              <div key={item.name} className="bg-white rounded-2xl shadow p-5">
                <img
                  src={item.image}
                  alt={item.name}
                  className="h-44 w-full object-cover rounded-xl bg-gray-100 mb-4"
                />

                <span className="text-xs bg-[#101b2d] text-white px-3 py-1 rounded-full font-bold">
                  {item.tag}
                </span>
                ...
                <button className="w-full mt-4 border rounded-lg py-2 font-bold hover:bg-gray-100">
                  Add to Cart
                </button>
              </div>
            ))}
